...
async function getDominantColor(url: string) {
  ...
  return (
    json.dominant_colors.vibrant?.hex || json.dominant_colors.vibrant_ligth?.hex
  );
}
  ...
  return (
    <div className="relative group mx-auto w-full max-w-72 px-8 pt-4">
      ...
      <div className="-mb-1 overflow-hidden py-4">
        <Scribble color={dominantColor} className="absolute inset-0 h-full" />
        <PrismicNextImage
      ...
      <div className="absolute inset-0 flex items-center justify-center opacity-0 transition-opacity duration-200 group-hover:opacity-100">
        <ButtonLink field={product.data.customizer_link} className="z-10">
          Customize
        </ButtonLink>
      </div>
    </div>
